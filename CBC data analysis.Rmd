...
## we can just use strain as the model.
## Similarly, I think that since the during-rw phase we have lab_or_rw differences,
## we can do lab_or_rw and strain interactions.
## Next, since we infect on the during phase, and we don't see the effects of
## infection until at the post-rw phase. We add the t_muris_dose only at the
## post-rw phase model.
...
# Pre-RW: strain only
neu_pre <- lmer(log(neu_number) ~ strain +
...
# During RW: environment x strain
neu_during <- lmer(log(neu_number) ~ lab_or_rw * strain +
                     (1 | cage_wedge) + (1 | cohort),
...
# Pre-RW emmeans: extract ~ strain, duplicate across dose and environment
emm_neu_pre <- as.data.frame(emmeans(neu_pre, ~ strain, type = "response")) %>%
  crossing(t_muris_dose = factor(c("Zero", "Low", "High"),
                                 levels = c("Zero", "Low", "High")),
           lab_or_rw    = factor(c("Lab", "RW"),
                                 levels = c("Lab", "RW"))) %>%
  mutate(phase = factor("Pre-RW", levels = c("Pre-RW", "During RW", "Post-RW")))

# During RW emmeans: extract ~ lab_or_rw * strain, duplicate across dose
emm_neu_during <- as.data.frame(emmeans(neu_during, ~ lab_or_rw * strain,
                                        type = "response")) %>%
  crossing(t_muris_dose = factor(c("Zero", "Low", "High"),
                                 levels = c("Zero", "Low", "High"))) %>%
...
# Pre-RW: strain only
lym_pre <- lmer(log(lym_number) ~ strain +
                  (1 | cage_wedge) + (1 | cohort),
                data = filter(cleaned_combined_cbc_data, phase == "Pre-RW"))
...
# During RW: environment x strain
lym_during <- lmer(log(lym_number) ~ lab_or_rw * strain +
                     (1 | cage_wedge) + (1 | cohort),
                   data = filter(cleaned_combined_cbc_data, phase == "During RW"))
...
emm_lym_pre <- as.data.frame(emmeans(lym_pre, ~ strain, type = "response")) %>%
  crossing(t_muris_dose = factor(c("Zero", "Low", "High"),
                                 levels = c("Zero", "Low", "High")),
           lab_or_rw    = factor(c("Lab", "RW"),
                                 levels = c("Lab", "RW"))) %>%
...
emm_lym_during <- as.data.frame(emmeans(lym_during, ~ lab_or_rw * strain,
                                        type = "response")) %>%
  crossing(t_muris_dose = factor(c("Zero", "Low", "High"),
                                 levels = c("Zero", "Low", "High"))) %>%
  mutate(phase = factor("During RW", levels = c("Pre-RW", "During RW", "Post-RW")))
...
# Pre-RW: strain only
mon_pre <- lmer(log(mon_number) ~ strain +
                  (1 | cage_wedge) + (1 | cohort),
                data = filter(cleaned_combined_cbc_data, phase == "Pre-RW"))
...
# During RW: environment x strain
mon_during <- lmer(log(mon_number) ~ lab_or_rw * strain +
                     (1 | cage_wedge) + (1 | cohort),
                   data = filter(cleaned_combined_cbc_data, phase == "During RW"))
...
emm_mon_pre <- as.data.frame(emmeans(mon_pre, ~ strain, type = "response")) %>%
  crossing(t_muris_dose = factor(c("Zero", "Low", "High"),
                                 levels = c("Zero", "Low", "High")),
           lab_or_rw    = factor(c("Lab", "RW"),
                                 levels = c("Lab", "RW"))) %>%
...
emm_mon_during <- as.data.frame(emmeans(mon_during, ~ lab_or_rw * strain,
                                        type = "response")) %>%
  crossing(t_muris_dose = factor(c("Zero", "Low", "High"),
                                 levels = c("Zero", "Low", "High"))) %>%
  mutate(phase = factor("During RW", levels = c("Pre-RW", "During RW", "Post-RW")))
...
# Pre-RW: strain only (log1p for zeros)
eos_pre <- lmer(log1p(eos_number) ~ strain +
                  (1 | cage_wedge) + (1 | cohort),
                data = filter(cleaned_combined_cbc_data, phase == "Pre-RW"))
...
# During RW: environment x strain
eos_during <- lmer(log1p(eos_number) ~ lab_or_rw * strain +
                     (1 | cage_wedge) + (1 | cohort),
                   data = filter(cleaned_combined_cbc_data, phase == "During RW"))
...
emm_eos_pre <- as.data.frame(emmeans(eos_pre, ~ strain)) %>%
...
emm_eos_during <- as.data.frame(emmeans(eos_during, ~ lab_or_rw * strain)) %>%
  mutate(response = expm1(emmean),
         lower.CL = expm1(lower.CL),
         upper.CL = expm1(upper.CL)) %>%
  crossing(t_muris_dose = factor(c("Zero", "Low", "High"),
...
emm_eos_post <- as.data.frame(emmeans(eos_post, ~ lab_or_rw * t_muris_dose * strain)) %>%
  mutate(response = expm1(emmean),
         lower.CL = expm1(lower.CL),
         upper.CL = expm1(upper.CL)) %>%
  mutate(phase = factor("Post-RW", levels = c("Pre-RW", "During RW", "Post-RW")))
...
# Pre-RW: strain only (log1p for zeros)
bas_pre <- lmer(log1p(bas_number) ~ strain +
                  (1 | cage_wedge) + (1 | cohort),
                data = filter(cleaned_combined_cbc_data, phase == "Pre-RW"))
...
# During RW: environment x strain
bas_during <- lmer(log1p(bas_number) ~ lab_or_rw * strain +
                     (1 | cage_wedge) + (1 | cohort),
                   data = filter(cleaned_combined_cbc_data, phase == "During RW"))
...
# log1p back-transform: expm1() reverses log1p()
emm_bas_pre <- as.data.frame(emmeans(bas_pre, ~ strain)) %>%
  mutate(response = expm1(emmean),
         lower.CL = expm1(lower.CL),
         upper.CL = expm1(upper.CL)) %>%
  crossing(t_muris_dose = factor(c("Zero", "Low", "High"),
...
emm_bas_during <- as.data.frame(emmeans(bas_during, ~ lab_or_rw * strain)) %>%
  mutate(response = expm1(emmean),
         lower.CL = expm1(lower.CL),
         upper.CL = expm1(upper.CL)) %>%
  crossing(t_muris_dose = factor(c("Zero", "Low", "High"),
...
emm_bas_post <- as.data.frame(emmeans(bas_post, ~ lab_or_rw * t_muris_dose * strain)) %>%
  mutate(response = expm1(emmean),
         lower.CL = expm1(lower.CL),
         upper.CL = expm1(upper.CL)) %>%
  mutate(phase = factor("Post-RW", levels = c("Pre-RW", "During RW", "Post-RW")))
